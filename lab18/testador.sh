...
if [ "$accept" == "y" ]; then
    ...
    if  (( $test != 200 )); then
        ...
        while [ true ]; do
            ...
            # Entrada
            curl https://susy.ic.unicamp.br:9999/mc102qrst/$lab/dados/arq$cur.in1 --insecure -O -s -L
            curl https://susy.ic.unicamp.br:9999/mc102qrst/$lab/dados/arq$cur.in2 --insecure -O -s -L
            ...
            if grep -qF "$string" arq$cur.in1; then
                rm -f arq$cur.in1 arq$cur.in2 arq$cur.res
                break
            else
                ((i++))
            fi
